    ...
    <div>
        <div class="cart-list-main flex" v-for="item in cartList">
            <div class="cart-list-left flex">
                <input type="checkbox" class="check" :class="{active:item.checked}" @click="checkClick(item)">
    ...
    export default {
        ...
        methods: {
            checkClick(item) {
                ...
                this.$bus.$emit('OnIsCheck', !(this.$store.state.cartList.find(item => !item.checked)))
            }
        }
    }
</script>
